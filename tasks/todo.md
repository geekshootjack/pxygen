...
- [x] Interpret `input depth` / `output depth` relative to the provided input root
...
- Refactored depth handling so `input depth` and `output depth` are now purely relative to the provided input root (`0` = input folder); directory mode, JSON mode, WebUI text, and CLI docs were updated together, and the last legacy absolute-depth compatibility path was removed.
- Deferred the `Input -> Output` terminal structure preview after reverting the first demo attempt, because its preview-only control flow exited the CLI instead of remaining part of the real run experience.
